...
class Pedido {
    ...
    public function atualizarStatus($id, $tipo, $valor) {
        // SEC-01: Whitelist explícita para prevenir SQL Injection via interpolação de coluna
        $colunasPermitidas = ['pagamento' => 'status_pagamento', 'entrega' => 'status_entrega'];
        if (!isset($colunasPermitidas[$tipo])) {
            throw new Exception("Tipo de atualização inválido.");
        }
        $coluna = $colunasPermitidas[$tipo];

        if ($tipo === 'entrega' && $valor === 'Entregue') {
            $sql = "UPDATE pedidos SET {$coluna} = ?, entregue_em = NOW() WHERE id = ?";
        } else {
    ...
    /**
     * SEC-03: Busca o último pedido de semanas anteriores do usuário.
     */
    public function buscarUltimoPedidoAnterior($usuario_id) {
    ...
    public function processarCarrinho($carrinho) {
        $total_calculado = 0;
        $itens_processados = [];
    ...
    public function criarPedido($usuario_id, $total_calculado, $forma_pagamento, $itens_processados, $transacao_id = null, $status_pagamento = 'Pendente') {
        try {
