...
async function carregarEntregadores() {
    ...
    try {
        const res = await fetch('api_admin_entregadores_v2.php');
        const json = await res.json();
        if (json.success) {
            tbody.innerHTML = '';
            if (json.data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" style="text-align:center">Nenhum entregador cadastrado.</td></tr>';
                return;
            }
            json.data.forEach(e => {
                tbody.innerHTML += `<tr>
                            ...
                            <td style="text-align:right">
                                <button class="btn btn-danger" onclick="excluirEntregador(${e.id})">🗑️ Excluir</button>
                            </td>
                        </tr>`;
            });
        } else {
            tbody.innerHTML = '<tr><td colspan="4">Erro ao carregar entregadores.</td></tr>';
        }
    } catch (error) {
...
async function salvarEntregador() {
    const dados = {
        ...
        senha: document.getElementById('entSenha').value,
    ...
    const btn = document.getElementById('btnSalvarEntregador');
