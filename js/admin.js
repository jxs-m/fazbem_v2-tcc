        ...
        if (json.success) {
            ...
            json.data.forEach(p => {
                ...
                if (p.status_pagamento === 'Pago') {
                    btnComprovante = `<a href="${window.getAbsoluteUrl('comprovante.php')}?tipo=pedido&id=${p.id}" target="_blank" class="btn" style="background:#1d4ed8; color:white; padding:6px 10px; border-radius:4px; text-decoration:none; font-size:12px; display:inline-flex; align-items:center; justify-content:center; white-space:nowrap;">Comprovante</a>`;
                }
